...
class KafkaProducerService(
  ...
  fun sendUserEvent(userEvent: UserEvent): Mono<Void> {
    return try {
  ...
  fun sendOrderEvent(orderEvent: OrderEvent): Mono<Void> {
    return try {
  ...
  fun sendNotificationEvent(notificationEvent: NotificationEvent): Mono<Void> {
    return try {
  ...
  fun generateSampleUserEvent(userId: String, eventType: UserEventType): UserEvent {
    return UserEvent(
      userId = userId,
      eventType = eventType,
  ...
  fun generateSampleOrderEvent(orderId: String, userId: String, eventType: OrderEventType): OrderEvent {
    return OrderEvent(
      orderId = orderId,
      userId = userId,
      eventType = eventType,
